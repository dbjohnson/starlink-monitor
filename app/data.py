    ...
    if DATA["starlink_history"]["timestamp"]:
        ...
        for i, ts2 in enumerate(DATA["starlink_status"]["timestamp"]):
            if ts2 > ts:
                return {
                    k: v
                    + DATA["starlink_status"].get(
                        k, [None] * len(DATA["starlink_status"]["timestamp"])
                    )[i:]
...
def _trim_buffer(b, secs_history=BUFFER_SIZE_SECS):
    ...
    for i, ts in enumerate(b["timestamp"]):
        if ts >= time.time() - secs_history:
            return defaultdict(list, {k: v[i:] for k, v in b.items()})
    else:
    ...
    def smooth(k, vals):
        ...
        if stride <= 1:
            ...
            try:
                ...
                # non-numeric type
                return [vals[i] for i in range(start, len(vals), stride)]

    return {k: smooth(k, v) for k, v in b.items()}
...
def broadcast(socketio, secs_history, update_rate=BROADCAST_RATE_SECS):
    ...
    BROADCAST_LOOPS.append(
        scheduler.repeat(broadcast, interval=datetime.timedelta(seconds=update_rate))
    )
...
def start_polling():
    ...
    scheduler.repeat(
        _update_speedtest, interval=datetime.timedelta(minutes=SPEEDTEST_REFRESH_MINS)
    )
